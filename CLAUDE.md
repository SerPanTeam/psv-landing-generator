...
| Padding не применяется | CSS класс `.pb-XXX` не существует | Добавить класс в sections.css или использовать существующее значение |

### КРИТИЧНО: Проверка padding классов

**Проблема:** `paddingTop`/`paddingBottom` в конфиге генерируют классы `pt-XXX`/`pb-XXX`, но CSS содержит только **предопределённый набор значений**.

**Симптом:** Отступ не появляется или исчезает после изменения значения.

**Решение:**
1. Перед использованием нового значения padding — проверить наличие класса в `css/sections.css`
2. Если класса нет — добавить: `.pb-XXX { padding-bottom: XXXpx !important; }`
3. Существующие классы: `pb-0, pb-38, pb-56, pb-72, pb-80, pb-86, pb-88, pb-89, pb-92, pb-96, pb-98, pb-103, pb-104, pb-106, pb-112, pb-115, pb-120, pb-137, pb-142, pb-146, pb-172, pb-195, pb-208`

**Чеклист:**
- [ ] Новое значение padding? → Проверить/добавить CSS класс
- [ ] Grep: `\.pb-XXX` в css/sections.css

### КРИТИЧНО: Секции с выступающими декорами (absolute positioned)

**Проблема:** Декоративные элементы с `position: absolute` и отрицательными `top`/`bottom` выступают за границы секции. padding считается от контейнера, а не от декора.

**Пример - gallery-decorated:**
- CSS: `padding: 40px 0 80px` (дефолт)
- Декор top-right: `top: -100px` → выступает на 100px вверх
- Декор bottom-left: `bottom: -100px` → выступает на 100px вниз

**Формула расчёта:**
```
// Для верхнего декора (предыдущая секция → текущая):
paddingBottom предыдущей = Figma_gap_до_декора + (выступ_декора - padding_top_текущей)
                        = 86 + (100 - 40) = 146px

// Для нижнего декора (текущая → следующая):
paddingBottom текущей = Figma_gap_от_декора + выступ_декора
                      = 72 + 100 = 172px
```

**Правило:** Если секция имеет абсолютно позиционированные декоры с отрицательным top/bottom, добавить выступ к padding.
...
## КРИТИЧНО: Проверка CSS переменных против Figma

### Урок #1: Цвета ДОЛЖНЫ точно совпадать с Figma!

**Проблема которую мы нашли:**
```css
/* БЫЛО (НЕПРАВИЛЬНО): */
--color-text-primary: #3D3D3D;  /* "почти чёрный" */

/* СТАЛО (ПРАВИЛЬНО): */
--color-text-primary: #000000;  /* Figma: text-black */
```

**Почему это критично:**
- `#3D3D3D` визуально похож на чёрный, но это НЕ чёрный
- Figma показывает `text-black` = это ВСЕГДА `#000000`
- Ошибка влияет на ВСЕ страницы, использующие переменную

**Как избежать:**
1. **НЕ ДОВЕРЯТЬ** существующим значениям в variables.css
2. **ВСЕГДА** проверять через `get_design_context` что реально в Figma
3. `text-black` в Figma = `#000000` (не #3D3D3D, не #333333, не #1A1A1A)
4. `text-white` в Figma = `#FFFFFF`

**Чеклист при старте работы над лендингом:**
- [ ] Проверить `--color-text-primary` = Figma text color
- [ ] Проверить `--color-bg-primary` = Figma background color
- [ ] Проверить `--color-button-primary` = Figma button color
- [ ] Проверить `--color-accent` = Figma accent color

---

## КРИТИЧНО: Проверять ВСЕ элементы секции, не только новые!

### Урок #2: Не забывать про существующие элементы

**Проблема которую мы пропустили:**
- Добавляя backdrop и caption под фото в About V5
- Забыли проверить отступ между "Über mich" (label) и текстом (paragraphs)
- В итоге `margin-bottom` для `.about--v5 .about__label` отсутствовал

**Почему пропустили:**
- Фокус был на НОВЫХ элементах (backdrop, nameUnderImage)
- Существующие элементы (label → text) казались "уже работающими"
- Но V5 переопределяет font-size/weight для label БЕЗ margin-bottom!

**Правило:**
При добавлении variant (v5, rounded, etc.) для существующей секции — проверять ВСЕ элементы:
- [ ] Заголовок секции (label) — font + margin
- [ ] Подзаголовок (subtitle) — font + margin
- [ ] Основной текст (text/paragraphs) — font + margin/gap
- [ ] Изображения — размеры + отступы
- [ ] Кнопки — размеры + отступы
- [ ] Декоративные элементы — если есть

**Формула расчёта gap от Figma:**
```
margin-bottom = element2.y - element1.y - element1.fontSize
```
Пример: `8104 - 8005 - 45 = 54px` → используем `margin-bottom: 50px`
